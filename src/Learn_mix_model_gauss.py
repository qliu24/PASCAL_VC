...
def learn_mix_model_gauss(categroy, K=4, kappa=5):
    ...
    savename = os.path.join(root_dir,'mix_model','mmodel_{}_K{}_notrain_gauss.pickle'.format(category, K))
    ...
    all_mus = [None for kk in range(K)]
    all_sigmas = [None for kk in range(K)]
    all_N = [0 for kk in range(K)]
    for kk in range(K):
        data_kk = np.array(all_train[kk])
        all_mus[kk] = np.mean(data_kk,axis=0)
        all_sigmas[kk] = np.std(data_kk,axis=0)
    ...
    with open(savename, 'wb') as fh:
        pickle.dump([all_mus, all_sigmas, all_priors], fh)
            

if __name__=='__main__':
    for category in all_categories:
        learn_mix_model_gauss(category)
